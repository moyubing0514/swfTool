	...
	public class SWFPrinter extends AbstractTagVisitor
	{
		...
		public function visitShape(shape:Shape):void
		{
			if (shape is ShapeWithStyle) {
				//trace("ShapeWithStyle(shape).fillStyles.lineStyles=",ShapeWithStyle(shape).fillStyles.numFillStyles);
				//trace("ShapeWithStyle(shape).lineStyles.numLineStyles=",ShapeWithStyle(shape).lineStyles.numLineStyles);
			...
			for each (var record:ShapeRecord in shape.shapeRecords) {
				switch (record.type) {
					...
					case ShapeRecordTypeConstants.STRAIGHT_EDGE:
						...
						trace("<StraightEdgeRecord(record).verticalLine>=",StraightEdgeRecord(record).verticalLine);
						break;
		...
		public function visitFillStyleArray(styles:FillStyleArray):void
		{
			...
			for each (var style:FillStyle in styles.fillStyles) {
				...
				// trace(style.bitmapMatrix.translateY);
				// trace(style.bitmapMatrix.scaleX);
				// trace(style.bitmapMatrix.scaleY);
				// trace(style.bitmapMatrix.translateY);
				// trace(style.bitmapMatrix.translateY);
				// trace(style.bitmapMatrix.translateY);
				// trace(style.bitmapMatrix.translateY);
